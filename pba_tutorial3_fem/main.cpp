...
// model choices
int mat_model = MaterialModel::LINEAR;
...
void update_force()
{
	...
	// update forces
	glm::mat3 H(1.0);
	H = -e.undeformed_volume * P * glm::transpose(e.reference_shape_matrix_inverse);
	sim_mesh.nodes[tet[base_index + 0]].force += H[0];
	sim_mesh.nodes[tet[base_index + 1]].force += H[1];
	sim_mesh.nodes[tet[base_index + 2]].force += H[2];
	sim_mesh.nodes[tet[base_index + 3]].force += -H[0] - H[1] - H[2];
    } // for (int i = 0; i < element_count; ++i) {  
}
void update_physics(float dt)
{
    ...
    // Second force calculation for Trapazoidal method
    if (model == IntegrationModel::TRAPAZOIDAL) {
        update_force();
        for (int i = 0; i < node_count; ++i) {
            node_t &node = sim_mesh.nodes[i];
