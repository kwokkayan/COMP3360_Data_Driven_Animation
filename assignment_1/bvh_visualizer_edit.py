...
'''
# helper function -- creates a cube at the x y z location
# reference: add_cube() from example.py in tutorial 2
def add_cube(location=[0, 0, 0], size=1, name="cube_joint"):
    ...
    c_obj.name = name
    return c_obj
# parent_obj should be 0 at start (pointing to self)
# added this_id to keep track of the this offset.
# reference: build_reset_skeleton() from this file.
def build_joint_cubes(offsets, parents, names, this_id, parent_obj, all_obj):
    offset = mathutils.Vector(offsets[this_id]) # get current cube's offset
    if parent_obj is not None: # add location of parent if exists --> attain current cube's global position 
        offset += parent_obj.location
    new_cube = add_cube(offset, 0.5, names[this_id] + "_cube") # spawn new cube :3
    all_obj.append(new_cube) # add it to all_obj (no idea what this does :shrug:)
    for i in range(len(parents)): # preorder tree traverse
        if parents[i] == this_id:
            build_joint_cubes(offsets, parents, names, i, new_cube, all_obj)
...
* 
'''
# references for rotating a point with quaternions:
...
# Added parameters positions and offsets for translation and rotation purposes respectively
def set_cube_animation(joints, positions, offsets, rotations, parents):
    global_position = np.zeros((rotations.shape[0], rotations.shape[1], 3))
    for frame in range(rotations.shape[0]):
        for cubeId in range(rotations.shape[1]):
            global_position[frame, cubeId] = offsets[cubeId] # get current cube's offset
            parent = parents[cubeId]
            while parent != -1: # rotate the offset with respect to parent
                rot = R.from_quat(np.roll(rotations[frame, parent], -1)) # get parent rotation
                new_offset = rot.apply(global_position[frame, cubeId]) # rotate this offset
                global_position[frame, cubeId] = mathutils.Vector(new_offset) # update offset
                parent = parents[parent] # go to parent's parent
            if parents[cubeId] != -1: # not root => add location of parent
                global_position[frame, cubeId] += mathutils.Vector(global_position[frame, parents[cubeId]])
            else: # is root => set translation of this frame
                global_position[frame, cubeId] += mathutils.Vector(positions[frame, cubeId])
            #add frame
            joints[cubeId].location = mathutils.Vector(global_position[frame, cubeId])
            joints[cubeId].keyframe_insert(data_path='location', frame=frame)
...
'''
rotations, positions, offsets, parents, names, frametime = load(filename='D://github/repo/COMP3360_Data_Driven_Animation/assignment_1/data/motion_files/motion_walk.bvh')
...
# new code
all_cubes = []
build_joint_cubes(offsets, parents, names, 0, None, all_cubes) # create cubes
# new code ends
all_joints = []
for j in range(rotations.shape[1]):
    name = names[j]
...
# new code
set_cube_animation(all_cubes, positions, offsets, rotations.qs, parents) # animate the cubes
# new code ends
